...
if [ -n "${CLOUDRON_APP_ORIGIN:-}" ]; then
  ...
  chown "$APP_UID:$APP_GID" /app/data /app/data/attachments

  secrets_file=/app/data/secrets.env
  if [ ! -f "$secrets_file" ]; then
    echo "==> first run: generating persistent Bloom secrets"
    umask 077
    {
      printf 'SECRET_KEY=%s\n' "$(python3 -c 'import secrets; print(secrets.token_hex(32))')"
      printf 'SERVICE_TOKEN_PEPPER=%s\n' "$(python3 -c 'import secrets; print(secrets.token_hex(32))')"
  ...
  echo "==> applying Bloom database migrations"
  gosu "$APP_USER" alembic upgrade head
else
  mkdir -p /app/attachments
  chown "$APP_UID:$APP_GID" /app/attachments
fi
