...
def _build_harness():
    ...
    harness = ArtefactVisibilityHarness(
        client=client, session_maker=session_maker, actor_id=actor_id
    )
    return harness, engine


async def _seed_artefact_visibility_data(session_maker: async_sessionmaker[AsyncSession]):
    async with session_maker() as session:
        external = User(
        ...
        project = Project(name="Artefact Visibility", prefix="ARV", description="Visibility")
        session.add(project)
        await session.flush()
...
def test_external_related_returns_404_for_internal_requirement():
    harness, engine = _build_harness()
    try:
        seeded = harness.run(_seed_artefact_visibility_data(harness.session_maker))
        harness.act_as(seeded["external"])

        hidden = harness.client.get(
            f"/api/artefacts/requirement/{seeded['internal_requirement'].id}/related"
        )
        assert hidden.status_code == 404, hidden.text

        visible = harness.client.get(
            f"/api/artefacts/requirement/{seeded['customer_requirement'].id}/related"
        )
