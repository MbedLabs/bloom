...
Copies every file in the local attachment directory into the configured bucket under
the same storage name, then checks each object's size against the local file. Files
...
def main(argv: list[str] | None = None) -> int:
    ...
    migrate = commands.add_parser("migrate", help="copy the local attachment files to S3")
