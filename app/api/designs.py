...
async def update_design_item(
    ...
    await audit_visibility_change(db, "design", item, previous_visibility)
    ...
    if should_log_generic_document_update(fields_set):
        await log_artefact_activity(
            ...
            item.id,
            "updated",
            updated_summary(current_user.full_name, "design item", item.design_id, fields_set),
        )
    return _design_response(item)
...
async def delete_design_item(
    ...
    await audit_artefact_deleted(db, "design", item)
