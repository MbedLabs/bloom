...
async def import_test_cases(
    ...
    A row whose tc_id already exists in the project updates that test case; a new or
    absent tc_id creates one with a freshly minted id. Returns created/updated/skipped
    counts with row-level errors.
    """
    project = (
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if existing:
            ...
            new_tc_id = await next_doc_id(
                db, TestCase, TestCase.tc_id, project_id, project.prefix, "TC"
            )
            kwargs = dict(
