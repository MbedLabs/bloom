...
REFRESH_COOKIE_NAME = "bloom_refresh_token"
...
async def refresh(
    ...
    refresh token (single-use). A replayed token (already rotated by the real
    client) is marked used and rejected."""
...
async def request_email_change(
    ...
    if current_user.role == UserRole.admin:
        current_user.email_change_status = "awaiting_current_confirmation"
        ...
        confirm_link = (
            f"{settings.FRONTEND_BASE_URL.rstrip('/')}/confirm-email-change#token={token}"
        )
        try:
            send_email_change_authorization_email(
...
async def confirm_email_change(
    ...
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    if not user.pending_email or user.email_change_status not in {
    ...
    if user.email_change_status == "awaiting_current_confirmation":
        user.email_change_status = "awaiting_confirmation"
        new_mailbox_token = await create_user_token(
            db,
            user_id=user.id,
            purpose=UserTokenPurpose.email_change,
            ttl_hours=settings.EMAIL_VERIFICATION_TOKEN_TTL_HOURS,
            created_by_user_id=user.id,
            target_email=candidate.target_email,
        )
        confirm_link = (
            f"{settings.FRONTEND_BASE_URL.rstrip('/')}/confirm-email-change"
            f"#token={new_mailbox_token}"
        )
        try:
            send_email_change_email(
                to_email=candidate.target_email,
                full_name=user.full_name,
                old_email=user.email,
                new_email=candidate.target_email,
                confirm_link=confirm_link,
            )
        except MailConfigurationError as exc:
            raise HTTPException(status_code=503, detail=str(exc)) from exc
