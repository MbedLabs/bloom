...
async def list_risk_items(
    ...
    base = apply_external_visibility_filter(base, RiskItem, current_user)
...
async def get_risk_item(
    ...
    item = (
        await db.execute(
            apply_external_visibility_filter(
                select(RiskItem).where(RiskItem.id == risk_id), RiskItem, current_user
            )
        )
    ).scalar_one_or_none()
    if not item:
...
async def update_risk_item(
    ...
    await audit_visibility_change(db, "risk", item, previous_visibility)
    ...
    if should_log_generic_document_update(fields_set):
        await log_artefact_activity(
            ...
            item.id,
            "updated",
            updated_summary(current_user.full_name, "risk item", item.risk_id, fields_set),
        )
    return _risk_response(item)
...
async def delete_risk_item(
    ...
    await audit_artefact_deleted(db, "risk", item)
