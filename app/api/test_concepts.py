...
async def list_test_concepts(
    ...
    result = await db.execute(
        select(TestConcept).where(TestConcept.project_id == project_id).order_by(TestConcept.created_at.desc())
    )
...
async def create_test_concept(
    ...
    concept_id = await next_doc_id(db, TestConcept, TestConcept.concept_id, data.project_id, project.prefix, "TCO")
    item = TestConcept(
    ...
    await log_artefact_activity(db, "test-concept", item.id, "created", f"{current_user.full_name} created test concept {item.concept_id}")
...
async def get_test_concept(
    ...
    item = (await db.execute(select(TestConcept).where(TestConcept.id == concept_id))).scalar_one_or_none()
    if not item:
...
async def update_test_concept(
    ...
    await db.flush()
    await db.refresh(item)
    await log_artefact_activity(db, "test-concept", item.id, "updated", f"{current_user.full_name} updated test concept {item.concept_id}")
    return _response(item)
...
async def delete_test_concept(
    ...
    if not item:
        raise HTTPException(status_code=404, detail="Test concept not found")
    await log_artefact_activity(db, "test-concept", item.id, "deleted", f"{current_user.full_name} deleted test concept {item.concept_id}")
