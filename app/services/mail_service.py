...
def send_email(
    *, to_email: str, subject: str, text_body: str, html_body: str | None = None
    ...
    message["From"] = f"{BLOOM_FROM_NAME} <{settings.SMTP_FROM_EMAIL}>"
    message["To"] = to_email
...
def send_invite_email(*, to_email: str, full_name: str, invite_link: str) -> None:
    ...
    send_email(
        to_email=to_email,
        subject=f"You're invited to {settings.BLOOM_APP_NAME}",
...
def send_email_change_email(
    *,
    ...
    send_email(
        to_email=to_email,
        subject=f"Confirm your approved email change for {settings.BLOOM_APP_NAME}",
...
def send_email_change_authorization_email(
    *,
    ...
    send_email(
        to_email=to_email,
        subject=f"Authorize your email change for {settings.BLOOM_APP_NAME}",
...
def send_admin_welcome_email(*, to_email: str, full_name: str, login_link: str) -> None:
    ...
    send_email(
        to_email=to_email,
        subject=f"You are the administrator of {settings.BLOOM_APP_NAME}",
...
def send_access_request_email(
    *,
    ...
    send_email(
        to_email=to_email,
        subject=f"Access requested on {settings.BLOOM_APP_NAME}: {resource}",
