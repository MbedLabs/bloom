...
export interface User {
  ...
  email_change_status?: 'requested' | 'awaiting_current_confirmation' | 'awaiting_confirmation' | null
...
/** How the registry is narrowed. Every one of these is applied by the server. */
export interface DocListParams {
  type?: string[]
  status?: string[]
  q?: string
  priority?: string
  /** A user id, or `assigned` / `unassigned`. */
  reviewer?: string
  links?: 'linked' | 'unlinked' | 'incoming' | 'outgoing' | 'suspect' | 'clean'
  createdFrom?: string
  createdTo?: string
  updatedFrom?: string
  updatedTo?: string
  ...
  /** Only these documents, as `TYPE:row_id` pairs — the shape a link stores. */
  keys?: string[]
}
...
export const docsApi = {
  ...
  list: async (projectRef: string, params?: DocListParams) => {
    ...
    if (params?.links) query.set('links', params.links)
    ...
    if (params?.keys) params.keys.forEach(k => query.append('keys', k))
  ...
  /**
   * How many documents of each type the project holds, and how many suspect
   * links they carry.
   *
   * The topology draws one node per type and the project screen wants a single
   * count; both used to download every document in the project to work that
   * out, which at a thousand documents is a page load spent on eight numbers.
   */
  typeSummary: async (projectRef: string) => {
    const response = await api.get<DocTypeSummary>(`/projects/${projectRef}/doc-type-summary`)
    return response.data
  },
...
export const attachmentsApi = {
  list: async (documentId: number) => {
    const response = await api.get<DocumentAttachment[]>(`/documents/${documentId}/attachments`)
    return response.data
  },
  upload: async (documentId: number, file: File) => {
    const form = new FormData()
    form.append('file', file)
  ...
  remove: async (attachmentId: number) => {
