...
describe('changing your own email', () => {
  it('sends the new address with the current password', async () => {
    vi.mocked(client.authApi.requestEmailChange).mockResolvedValue({
      message: 'An administrator must approve this request.',
    } as never)
    renderSettings()
    ...
    expect(await screen.findByText(/an administrator must approve this request/i)).toBeTruthy()
  ...
  it('says an approved change is waiting on the new mailbox', async () => {
    currentUser = {
      ...user,
      pending_email: 'ada.new@example.com',
    ...
    expect(screen.getByText(/use the confirmation link sent to the new address/i)).toBeTruthy()
  })
