...
export default function TraceabilityMatrix() {
  ...
  const partialCoveragePercent = totalRequirements > 0
    ? Math.round((partial / totalRequirements) * 1000) / 10
    : 0
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        <SummaryCard
          ...
          value={`${partialCoveragePercent}%`}
          icon={AlertCircle}
          color={partialCoveragePercent > 0 ? 'text-amber-600' : 'text-emerald-600'}
        />
        <SummaryCard
          label="Uncovered"
          value={uncovered}
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-4 text-sm">
                <span className="text-muted-foreground">
                  {coverageReport.coverage_percent}% total coverage
                </span>
                <span className="text-red-600 font-medium">{coverageReport.uncovered} uncovered</span>
                <span className="text-amber-600 font-medium">{partialCoveragePercent}% partial coverage</span>
              </div>
            </div>
          </div>
