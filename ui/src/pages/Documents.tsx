...
export default function Documents() {
  ...
  // The address bar keeps the typed value so the input stays responsive and the
  ...
  const { data: docsData, isLoading } = useQuery({
    queryKey: [
      ...
      reviewerFilter, linkFilter, createdFrom, createdTo, updatedFrom, updatedTo,
      sortField, sortDir, page, pageSize, relatedTo, relatedRole, relatedDirection,
    ],
  ...
  // Narrowing the set can leave the reader on a page that no longer exists.
  useEffect(() => {
    setPage(0)
  }, [
    typeFilters, statusFilters, debouncedSearch, priorityFilter, reviewerFilter, linkFilter,
    createdFrom, createdTo, updatedFrom, updatedTo, pageSize, relatedTo,
  ])
            ...
            <div className="border-t border-border px-4 py-3 flex items-center justify-between text-sm bg-muted/20">
              <div className="flex items-center gap-3">
                <span className="text-muted-foreground">
                  {page * pageSize + 1}&ndash;{Math.min((page + 1) * pageSize, totalDocs)} of {totalDocs}
                </span>
