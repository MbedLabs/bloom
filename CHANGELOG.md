...
- **The results sync tells Bud which campaigns it reached.** `POST /api/campaigns/sync-results` answered only with a count and the identifiers it could not match, so Bud had no way to record where the results landed. The response now also names each campaign whose items the sync updated - numeric id, public id, name, and the address of its page in Bloom - which is what lets a Bud run link back to the campaign it came from. Nothing about what the endpoint writes has changed.
